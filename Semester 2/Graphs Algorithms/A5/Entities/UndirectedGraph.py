...
class UndirectedGraph:
    ...
    def is_clique(self, y, vertices):
        for x in vertices:
            if y not in self._dictNeighbours[x]:
                return False
        return True

    def max_clique(self, l, vertices, v):
        for y in self._dictNeighbours[v]:
            if y not in vertices:
                if self.is_clique(y, vertices):
                    vertices.append(y)
                    l = self.max_clique(l+1, vertices, v)
        return l
...
# g = UndirectedGraph(5)
# g.initialize_dictionaries_of_the_graph(6)
# g.add_edge(1, 2, 1)
# g.add_edge(1, 3, 1)
# g.add_edge(2, 3, 1)
# g.add_edge(3, 4, 1)
...
# g.add_edge(5, 2, 1)
# vertices = [1]
# print(g.max_clique(1, vertices, 1))