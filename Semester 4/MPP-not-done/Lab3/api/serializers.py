...
class BookSerializer2(serializers.ModelSerializer):
    class Meta:
        model = Book
        fields = ('id','isbn', 'title', 'author', 'genre', 'library', 'pages', 'price')
...
class GenreSerializer2(serializers.ModelSerializer):
    ...
    class Meta:
        model = Genre 
        fields = ('id','name', 'description', 'popularity', 'age_group', 'rating', 'books')

    def create(self, validated_data):
        books_data = validated_data.pop('books')
        genre = Genre.objects.create(**validated_data)
        for book_data in books_data:
            Book.objects.create(genre=genre, **book_data)
        return genre
