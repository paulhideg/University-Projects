...
#create a view to get the genres sorted by the total pages of books from that genre
class GenreStatsView(views.APIView):
    def get(self, request):
        ...
        for genre in genres:
            genreStats.append({
                'genre': genre.name,
                'totalPages': Book.objects.filter(genre=genre).aggregate(Sum('pages'))['pages__sum']
            })
        genreStats.sort(key=lambda x: x['totalPages'], reverse=True)
...
class GenresView2(views.APIView):
    def get(self, request):
        genres = Genre.objects.all()
